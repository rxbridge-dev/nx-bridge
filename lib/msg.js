...
const sms = (ranuxPro, m) => {
	try {
		if (m.key) {
			...
			m.sender = m.fromMe
				? ranuxPro.user.id.split(':')[0] + '@s.whatsapp.net'
				: m.isGroup
					? m.key.participant
					: m.key.remoteJid
		}
		...
		if (m.message) {
			...
			if (m.msg) {
				...
				if (m.quoted) {
					...
					m.quoted.fromMe = (m.quoted.sender || '').split('@')[0] === ranuxPro.user.id.split(':')[0]
					...
					m.quoted.react = (emoji) =>
						ranuxPro.sendMessage(m.chat, { react: { text: emoji, key: m.quoted.fakeObj.key } })
				}
			}
		...
		m.reply = (text, id = m.chat, option = { mentions: [m.sender] }) =>
			ranuxPro.sendMessage(id, { text, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })

		m.replyS = (sticker, id = m.chat, option = { mentions: [m.sender] }) =>
			ranuxPro.sendMessage(id, { sticker, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })

		m.replyImg = (img, text, id = m.chat, option = { mentions: [m.sender] }) =>
			ranuxPro.sendMessage(id, { image: img, caption: text, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })

		m.replyVid = (vid, text, id = m.chat, option = { mentions: [m.sender], gif: false }) =>
			ranuxPro.sendMessage(id, { video: vid, caption: text, gifPlayback: option.gif, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })

		m.replyAud = (aud, id = m.chat, option = { mentions: [m.sender], ptt: false }) =>
			ranuxPro.sendMessage(id, { audio: aud, ptt: option.ptt, mimetype: 'audio/mpeg', contextInfo: { mentionedJid: option.mentions } }, { quoted: m })

		m.replyDoc = (doc, id = m.chat, option = { mentions: [m.sender], filename: 'undefined.pdf', mimetype: 'application/pdf' }) =>
			ranuxPro.sendMessage(id, { document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
		...
		m.react = (emoji) =>
			ranuxPro.sendMessage(m.chat, { react: { text: emoji, key: m.key } })

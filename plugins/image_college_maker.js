...
cmd({
  ...
  filename: __filename,
}, async (ranuxPro, mek, m, { from, reply, quoted }) => {
  try {
    const imgs = [];
    ...
    if (m.message?.extendedTextMessage?.contextInfo?.quotedMessage?.albumMessage) {
      ...
      for (const item of album) {
        if (item.imageMessage) {
          const buff = await ranuxPro.downloadMediaMessage(item);
          imgs.push(buff);
        }
      }
    }
    ...
    await ranuxPro.sendMessage(
      from,
      { image: fs.readFileSync(outFile), caption: "🖼️ Collage created by *ranuxPro-MD*" },
      { quoted: mek }
    );
